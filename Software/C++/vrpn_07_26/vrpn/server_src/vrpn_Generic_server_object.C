...
#ifdef VRPN_USE_GTAB
int vrpn_Generic_Server_Object::setup_Tracker_GTab (char* &pch, char* line, FILE* config_file)
{
	/*
	 * Parses the section of the configuration file related to the device
		#DeviceClass DeviceName ListenPort
		Tracker_GTab GGG 7777
	 *
	...
	if(count < 1){
		fprintf(stderr,"Bad vrpn_Tracker_GTab line: %s\n",line);
		return -1;
	}

	// Make sure there's room for a new one:

	if (num_GTabs >= VRPN_GSO_MAX_GTABS) {
		fprintf(stderr,"Too many GPads in config file");
		return -1;
	}
	...
	// Open vrpn_Tracker_GTab:

	if(verbose){
		printf("Opening vrpn_Tracker_GTab: %s at port %d\n", s2, port);
	}

	if((GTabs[num_GTabs] = new vrpn_Tracker_GTab(s2, connection, port)) == NULL)
	{
		fprintf(stderr,"Can't create new vrpn_Tracker_GTab\n");
		return -1;
	}
...
#ifdef VRPN_USE_GTAB
	  else if (isit("vrpn_Tracker_GTab")) {
		  CHECK(setup_Tracker_GTab);
	  }
#endif	 
