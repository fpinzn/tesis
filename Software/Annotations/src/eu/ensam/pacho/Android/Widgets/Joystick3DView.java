...
//					Log.d(TAG, String.format("moveListener.OnMoved(%d,%d)", (int)userX, (int)userY));
	...
	private void reportVrpn() {
		//The method setVrpnId must be called after the constructor to enable the VRPN functionality
		VrpnClient.getInstance().send3DData(vrpnId,Float.parseFloat(userX+"")/50,Float.parseFloat(userY+"")/50,Float.parseFloat(userZ+"")/50);
	}
